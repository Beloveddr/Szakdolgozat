...
class EnemyEntity(pygame.sprite.Sprite):
    ...
    # raycasting
    # def cast_ray_to_player(self, obstacle_sprites, player_rect):
    #     line_rect = pygame.draw.line(self.display_surface, (255, 0, 0), self.pos, player_rect.center)
    #     if not line_rect.collidelist(obstacle_sprites):
    #         return True
    #     return False
    ...
    def create_path(self, to_spawn=False):
        ...
        if not to_spawn:
            ...
            self.last_position = self.player_coord()
        else:
            end_x, end_y = self.spawn_pos
        end = self.grid.node(end_x, end_y)

        # path
        # self.path, _ = self.pathfinder.find_path(start, end, self.grid)
        self.path = astar(self.matrix, start, end)
